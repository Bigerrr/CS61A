...
def hefty_hogs(player_score, opponent_score):
    ...
    if opponent_score == 0:
        ...
        current_point = player_score
        while opponent_score != 0:
            digit = opponent_score % 10
            func = digit_fn(digit)
            current_point = func(current_point)
            # print('Debug', f'digit = {digit} func = {func.__name__} score = {current_point}')
            opponent_score //= 10
        return current_point % 30
    # END PROBLEM 2
...
def hog_pile(player_score, opponent_score):
    ...
    if one_digit == opponent_score % 10:
        return one_digit
    else:
        return 0
    # END PROBLEM 4
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        ...
        num_rolls = current_strategy(player_score, opponent_score)
        points = take_turn(num_rolls, player_score, opponent_score, dice, goal)
        player_score += points
        player_score += hog_pile(player_score, opponent_score)
        score0, score1 = (player_score, opponent_score) if who == 0 else (opponent_score, player_score) 
        who = next_player(who)
    # END PROBLEM 5
...
def announce_lead_changes(score0, score1, last_leader=None):
    ...
    if score0 > score1:
        ...
        leader = None
    if leader == None or leader == last_leader:
        return leader, None
    else:
...
def hog_pile_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    hefty_points = hefty_hogs(score, opponent_score)
    if hog_pile(score + hefty_points, opponent_score) > 0:
        return 0
    else:
